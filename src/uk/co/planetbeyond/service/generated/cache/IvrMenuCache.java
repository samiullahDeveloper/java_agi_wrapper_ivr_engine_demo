 ...
 * @author afagot
 */
public final class IvrMenuCache 
{
	/**
	 * Unique instance of the cache manager.
	 */
	private static IvrMenuCache instance = new IvrMenuCache();

	/**
	 * Maximum number of IvrMenuBean objects that may be in the cache.
	 */
	private int maxCacheSize = 80;

	/**
	 * The number of IvrMenuBean objects currently in the cache.
	 */
	 ...
	 * objects that refer to the actual IvrMenuBean.
	 */
	...
	*/
	 ...
	 */
	public static IvrMenuCache getInstance() 
	{
		return instance;
	}

	/**
	 * A IvrMenuBean object is passed to this method for addition to the cache.
	 */
	public void addIvrMenu(IvrMenuBean bean) 
	{
		synchronized(updateLock)
		{ 
			Integer id = bean.getIvrMenuId();
			if (null == cache.get(id)) 
			{
				...
				else 
				{
					...
					if (maxCacheSize < currentCacheSize) 
					{
						...
						cache.remove(newLink.bean.getIvrMenuId());
						lru.next = null;
					} 
					else 
					{
	...
	/**
	 * Return the IvrMenuBean associated with the given key.
	 */
	public IvrMenuBean fetchIvrMenu(Integer id) 
	{
		LinkedList foundLink = (LinkedList) cache.get(id);
		if (null == foundLink) 
		{
			IvrMenuBean bean = null;
			try 
			{
				bean = IvrMenuManagerImpl.getInstance().loadByPrimaryKey(id);
			} 
	...
	/**
	 * Updates IvrMenuBean object in the cache.
	 */
	public void updateIvrMenu(IvrMenuBean bean) 
	{
		synchronized(updateLock)
		{
			// check if the given object is present in the cache
			if (isIvrMenuInCache(bean))  
			{
				Integer id = bean.getIvrMenuId();			
	...
	/**
	 * Returns true if IvrMenuBean object is in the cache. FALSE otherwise
	 */
	public boolean isIvrMenuInCache(IvrMenuBean bean) 
	{
		Integer id = bean.getIvrMenuId();
